...
function AppContent() {
  ...
  if (isAuthLoading) {
    return (
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-[#e8ecef] text-gray-900'} flex items-center justify-center font-mono`}>
        <div className="bg-white dark:bg-slate-900 border-4 border-black rounded-2xl p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-center">
  ...
  if (!isAuthLoading && isLoggedIn && !isAdminLoggedIn && globalSettings?.maintenanceMode && currentPage !== 'admin') {
    return (
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-[#e8ecef] text-gray-900'} flex items-center justify-center font-mono p-4`}>
        <div className={`w-full max-w-md bg-white dark:bg-slate-900 border-4 border-black rounded-2xl p-8 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] text-center space-y-4`}>
          <div className="text-4xl">🛠️</div>
          <h2 className="text-xl font-black uppercase">{t('maintenance.title')}</h2>
          <p className="text-xs text-gray-600 dark:text-gray-300 font-sans font-bold leading-relaxed">
